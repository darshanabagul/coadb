...
class Api extends CI_Controller {
			...
			if(!empty($orders)){
						...
						foreach($orders as $order){
							...
							if(empty($exists)){
								...
								if(!empty($data['orderDetails']) && $data['orderDetails'] != '[]'){
	...
	public function check_ss_eligible($order){
		...
		if($onetime == 1 && $is_fullfilled == 1 && $is_new == 1)
		return 1;
		else
		return 0;
	...
	public function get_cust_phone($subscriberContactDetails){
		$email = '';
		foreach($subscriberContactDetails as $value){
	...
	public function send_to_shipstation($orderDetails,$subscriberDetails){
		...
		$data['orderNumber'] = $orderDetails['orderNumber'].'/'.$subscriberDetails['accountNumber'];
		$data['orderKey'] = '';
		...
		$data['internalNotes'] = 'Contact No. '.$this->get_cust_phone($subscriberDetails['contact']);
		$data['gift'] = 'false';
		$data['giftMessage'] = '';
		$data['paymentMethod'] = '';
		$data['requestedShippingService'] = 'Priority Mail';
		$data['advancedOptions']['customField1'] = $subscriberDetails['accountNumber'];
		$data['advancedOptions']['customField2'] = $this->get_cust_phone($subscriberDetails['contact']);
...
// ];
	...
	public function checking_orders($username){
		//GoDishAdmin
		$data = $this->Common->getOrders($username);
		echo '<pre>';
		print_r($data);
		exit;
	}
	...
	public function custom_add($order){
